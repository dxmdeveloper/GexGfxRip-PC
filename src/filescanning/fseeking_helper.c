...
{
    while (*str2) {
	if (!*str1 || (toupper(*str1) != toupper(*str2)))
	    return 0;
	str1++;
	str2++;
    }
...
/// @brief used when g instruction is null or invalid. The switch ought to be broke after calling this function
static inline void _fscan_follow_pattern_continue_to_offset_pop(const char **pcurp, gexdev_stack32 *loopstackp)
{
    ...
    if (scopeend && lowlevelpopscopeend)
	pcurdest = MIN(scopeend, lowlevelpopscopeend);
    else
	pcurdest = (scopeend ? scopeend : lowlevelpopscopeend);

    if (pcurdest == NULL) {
	*pcurp = *pcurp + strlen(*pcurp);
	return;
    }

    for (; *pcurp != pcurdest; (*pcurp)++) {
	if (**pcurp == ';') {
	    gexdev_stack32_pop(loopstackp);
	    gexdev_stack32_pop(loopstackp);
	}
    }
}
...
#define DEST_TYPE_INTERNAL_VAR 1
    ...
    // dest param
    if (strstartswith_ci((*pcurp), "stdout") || strstartswith_ci((*pcurp), "print")) {
    ...
    if (destType == DEST_TYPE_INTERNAL_VAR && rcount >= ivarIndex + INTERNAL_VAR_CNT) {
	(*pcurp) = expr_end;
	return EXIT_FAILURE;
    }
	...
	switch (**pcurp) {
	case 'X':
	    sprintf(priformat, "%%0%uX ", bytes * 2);
	    break;
	case 'x':
	    sprintf(priformat, "%%0%ux ", bytes * 2);
	    break;
    ...
    switch (bytes) {
    case 1:
	successfully_read = fread(buf, 1, rcount, fp);
	break;
    case 2:
	successfully_read = fread_LE_U16(buf, rcount, fp);
	break;
    case 4:
	successfully_read = fread_LE_U32(buf, rcount, fp);
	break;
    }
    if (successfully_read < rcount) {
	free(buf);
	(*pcurp) = expr_end;
	return EXIT_FAILURE;
    }
	...
	case DEST_TYPE_FILE:
	    ...
	    case 1:
		if (isSigned)
		    printf(priformat, ((i8 *)buf)[i]);
		else
		    printf(priformat, ((u8 *)buf)[i]);
		break;
	    case 2:
		if (isSigned)
		    printf(priformat, ((i16 *)buf)[i]);
		else
		    printf(priformat, ((u16 *)buf)[i]);
		break;
	    case 4:
		if (isSigned)
		    printf(priformat, ((i32 *)buf)[i]);
		else
		    printf(priformat, ((u32 *)buf)[i]);
		break;
	    }
	    break;
	...
	case DEST_TYPE_INTERNAL_VAR:
	    ...
	    case 1:
		if (isSigned)
		    *(i32 *)&ivars[ivarIndex + i] = (i32)((i8 *)buf)[i];
		else
		    ivars[ivarIndex + i] = (u32)((u8 *)buf)[i];
		break;
	    case 2:
		if (isSigned)
		    *(i32 *)&ivars[ivarIndex + i] = (i32)((i16 *)buf)[i];
		else
		    ivars[ivarIndex + i] = (u32)((u16 *)buf)[i];
		break;
	    case 4:
		if (isSigned)
		    *(i32 *)&ivars[ivarIndex + i] = ((i32 *)buf)[i];
		else
		    ivars[ivarIndex + i] = ((u32 *)buf)[i];
		break;
	    }
	    break;
	}
    }
    if (destType == DEST_TYPE_FILE)
	printf("\n");
    if (buf)
	free(buf);
...
int fscan_follow_pattern(fscan_file_chunk *fch, const char pattern[], jmp_buf *error_jmp_buf)
{
	...
	switch (*pcur) {
	case 'g': {
	    u32val = fscan_read_gexptr(fch->ptrs_fp, fch->offset, error_jmp_buf);
	    if (!u32val)
		return EXIT_FAILURE;
	...
	case 'd': {
	    ...
	    u32 offset = fscan_read_gexptr(fch->ptrs_fp, fch->offset, error_jmp_buf);
	    if (!offset)
		return EXIT_FAILURE;
	    fseek(fch->data_fp, offset, SEEK_SET);
	} break;
	case 'r':
	    _fscan_follow_pattern_read(&pcur, fch->ptrs_fp, NULL, error_jmp_buf);
	    break;
	}
    }
    return EXIT_SUCCESS;
...
size_t fscan_follow_pattern_recur(fscan_file_chunk *fch, const char pattern[], void *pass2cb,
				  ...
				  jmp_buf **errbufpp)
{
    gexdev_stack32 offsetStack = { 0 };
    gexdev_stack32 loopStack = { 0 };
    ...
    gexdev_stack32_init(&offsetStack, 128);
    gexdev_stack32_init(&loopStack, 256);

    FSCAN_ERRBUF_CHAIN_ADD(errbufpp, fprintf(stderr, "err: fscan_follow_pattern_recur error"); gexdev_stack32_close(&offsetStack);
			   gexdev_stack32_close(&loopStack); gexdev_u32vec_close(&iterVec););
	...
	case 'g': {
	    //GOTO GEXPTR
	    u32 offset = fscan_read_gexptr(fch->ptrs_fp, fch->offset, *errbufpp);
	...
	case '[': {
	    // LOOP START
	    gexdev_stack32_push(&loopStack, (u32)(pcur - procpatternp)); // save pcur position
	    gexdev_stack32_push(&loopStack, 0); // new iteration
	    if (iterVec.size < loopStack.sp / 2)
		gexdev_u32vec_ascounter_inc(&iterVec, loopStack.sp / 2 - 1);
	} break;
	case ';': {
	    // LOOP BODY END
	    u32 popedIteration = gexdev_stack32_pop(&loopStack);
	    u32 popedPcurPos = gexdev_stack32_pop(&loopStack);
		...
		if (popedIteration + 1 >= repeats) {
		    // loop broken
		    gexdev_u32vec_pop_back(&iterVec);
		    break;
		}
	    }
	    // if the loop isn't over
	    pcur = procpatternp + popedPcurPos;
	    gexdev_stack32_push(&loopStack, popedPcurPos);
	    gexdev_stack32_push(&loopStack, popedIteration + (isNullTerminated ? 0 : 1));
	    gexdev_u32vec_ascounter_inc(&iterVec, loopStack.sp / 2 - 1);
	} break;
	case 'G': {
	    pcur += 1; // skip to the '{'
	    u32 offset = fscan_read_gexptr(fch->ptrs_fp, fch->offset, *errbufpp);
	    if (offset < fch->offset + fch->size / 4096 || offset >= fch->size + fch->offset) {
		if (!_fscan_follow_pattern_break_infinite_loop(&pcur, &loopStack, &offsetStack))
		    break;
		pcur = strFindScopeEnd(pcur, '}');
		break;
	    }
	    gexdev_stack32_push(&offsetStack, (u32)ftell(fch->ptrs_fp) - 4);
	    fseek(fch->ptrs_fp, offset, SEEK_SET);
	} break;
	case '}': {
	    // G scope end
	    u32 offset = gexdev_stack32_pop(&offsetStack);
	...
	case 'p': {
	    gexdev_stack32_push(&loopStack, (u32)ftell(fch->ptrs_fp));
	} break;
	case 'b': {
	    u32 offset = gexdev_stack32_pop(&loopStack);
	    if (offset) {
		fseek(fch->ptrs_fp, offset, SEEK_SET);
	    }
	} break;
	...
	case 'r':
	    _fscan_follow_pattern_read(&pcur, fch->ptrs_fp, internalVars, *errbufpp);
	}
    }

    gexdev_stack32_close(&offsetStack);
    gexdev_stack32_close(&loopStack);
