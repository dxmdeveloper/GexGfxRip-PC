...
struct gfx_palette gfx_create_palette(void *gex_palette)
{
    ...
    case 0xffffff00:
	newPalette.colors_cnt = 16;
	break;
    case 0xffffff01:
	newPalette.colors_cnt = 256;
	break;
...
uint8_t **gfx_draw_img_from_raw(const void *gfx_headers, const uint8_t bitmap_dat[])
{
    ...
    case 5:
	return gfx_draw_sprite(gfx_headers, bitmap_dat, 8, real_height, real_width);
    case 4:
	return gfx_draw_sprite(gfx_headers, bitmap_dat, 4, real_height, real_width);
    case 2:
	return (u8 **)gfx_draw_gex_bitmap_16bpp(gfx_headers, bitmap_dat, real_height, real_width);
    case 1:
	return gfx_draw_gex_bitmap(gfx_headers, bitmap_dat, 8, real_height, real_width);
    case 0:
	return gfx_draw_gex_bitmap(gfx_headers, bitmap_dat, 4, real_height, real_width);
    }
    return NULL;
}
...
u8 **gfx_draw_gex_bitmap(const void *chunk_headers, const u8 bitmap_dat[], uint8_t bpp, u32 min_width, u32 min_height)
{
    ...
    uint bitmap_offset = chunk.start_offset; // This works different than the game engine
...
void **gfx_draw_gex_bitmap_16bpp(const void *chunk_headers, const void *bitmap_dat, uint32_t min_width, uint32_t min_height)
{
    void **image = NULL;
...
u8 **gfx_draw_sprite(const void *chunk_headers_and_opmap, const u8 *bitmap_dat, u8 bpp, u32 min_width, u32 min_height)
{
	...
	for (uint i = 0; i < repeats; i++) {
	    ...
	    switch (optype) {
	    case 0:
		pixVal = bitmap_dat[pix_ind++ / (8 / bpp)];
		break;
	    case 1:
