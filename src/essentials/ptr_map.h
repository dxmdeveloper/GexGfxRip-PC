...
typedef struct gexdev_ptr_map {
    uint32_t (*index_compute_cb)(void * key);
    uint32_t max_index;
    size_t region_count;
    void ** mem_regions;
} gexdev_ptr_map;
...
void gexdev_ptr_map_init(gexdev_ptr_map * ptrmapp, uint32_t max_index, uint32_t (*index_compute_cb)(void * key));
...
void* gexdev_ptr_map_get(void * key);
...
int gexdev_ptr_map_set(void * key, void * pointer);