...
static inline void **p_gexdev_ptr_map_get_ptr_to_ptr(gexdev_ptr_map *ptrmapp, uint32_t absolute_index, bool allocRegionIfNotPresent)
{
...
int gexdev_ptr_map_init(gexdev_ptr_map *ptrmapp, uint32_t max_index, uint32_t (*index_compute_cb)(const void *))
{
	...
	ptrmapp->index_compute_cb = p_gexdev_ptr_map_default_cb;
...
void gexdev_ptr_map_close_all(gexdev_ptr_map *ptrmapp)
{
	...
	void **pp = p_gexdev_ptr_map_get_ptr_to_ptr(ptrmapp, index, false);
	if (pp && *pp) {
	    free(*pp);
	    *pp = NULL;
	}
...
void *gexdev_ptr_map_get(gexdev_ptr_map *ptrmapp, const void *key)
{
    ...
    void **pp = p_gexdev_ptr_map_get_ptr_to_ptr(ptrmapp, abs_index, false);
    return (pp ? *pp : NULL);
}
...
{
    uint32_t abs_index = ptrmapp->index_compute_cb(key);
    void **pp = p_gexdev_ptr_map_get_ptr_to_ptr(ptrmapp, abs_index, true);
    if (!pp)
	return EXIT_FAILURE;
    *pp = pointer;
    return EXIT_SUCCESS;
